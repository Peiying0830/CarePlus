// dashboard.js - Doctor Dashboard JavaScript
...
function updateWelcomeMessage() {
    ...
    if (welcomeText) {
        ...
        // Get doctor name from data attribute
        const doctorName = welcomeText.getAttribute('data-doctor-name') || '';
        if (doctorName) {
            // Extract last name
            const nameParts = doctorName.split(' ');
            const lastName = nameParts[nameParts.length - 1];
            welcomeText.innerHTML = `${greeting}, Dr. ${lastName}! 👨‍⚕️`;
        }
    }
}
...
function initAutoRefresh() {
    ...
    setInterval(function() {
        ...
        // Example:
        // fetch('ajax/dashboard_updates.php')
        //     .then(response => response.json())
        //     .then(data => updateDashboard(data));
        
        // For now, just check for upcoming appointments
...
function updateDashboard(data) {
    ...
    // Example: Show notification if new appointments
    if (data.new_appointments && data.new_appointments.length > 0) {
        showNotification(`${data.new_appointments.length} new appointment(s) pending confirmation`, 'info');
    }
}
